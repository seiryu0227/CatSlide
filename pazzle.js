...
///** ページが読み込まれたら実行 */
//window.onload = () => {
//	loadImage("column_24_mv.jpg").then(loadedImage => {
//		// 正解画像を390×390で描画する
...
function createSlidePazzleImage(seikaiImage, blankTile) {
	...
	image.onload = async () => {
		...
		// 完成状態を定義
		const completedState = [1, 2, 3, 4, 5, 6, 7, 8, 9];
		...
		function shuffleTiles(state, moves) {
			let emptyIndex = state.indexOf(9); // 空白の位置
			let currentState = [...state];

			for (let i = 0; i < moves; i++) {
				const possibleMoves = directions.map((dir) => {
					const newIndex = emptyIndex + dir.dy * 3 + dir.dx;
					if (newIndex >= 0 && newIndex < 9) {
						// 縦または横の移動が可能な場合
						if (Math.abs(newIndex % 3 - emptyIndex % 3) + Math.abs(Math.floor(newIndex / 3) - Math.floor(emptyIndex / 3)) === 1) {
							return newIndex;
						}
					}
					return null;
				...
				if (possibleMoves.length > 0) {
					...
					emptyIndex = randomMove; // 空白の位置を更新
				}
			}
		...
		// ランダムに生成した配置を取得
		const displayOrder = shuffleTiles(completedState, 100); // 100回移動
...
const checkResult = () => {
	...
	if (isCorrect && !message) {
		...
		// ここでtitle.htmlにリダイレクト
		message = true; // フラグを立てる
